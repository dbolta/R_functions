...
Original use case:
<br>Had 1e6 points in grid.
<br>Needed min distance for evry point on grid out to set of data collection points.

Result below performed fastest.
<br>Other tests included
<br>1. Straight dplyr without multiprocessing
<br>2. Multidplyr, created memory issues
<br>3. Converting to matrix and using matrix operations
<br>4. Other flavors of lapply or purrr::map
```{r eval=FALSE}
library(tidyverse)
library(parallel)
